...
def compileSource(buildEnv, Boolean buildUnittestTarball, String enterpriseUrl, String outDir, String envName, Boolean Reliable) {
  try {
    ...
    BUILDSCRIPT="security list-keychains; export DYLD_FALLBACK_LIBRARY_PATH=/System/Library/Frameworks/CoreFoundation.framework/Versions/A/CoreFoundation:/System/Library/Frameworks/Security.framework/Versions/A/Security;otool -l /usr/bin/codesign;  /usr/bin/codesign --verbose=4 --deep -f -s 'W7UC4UQXPV' '/Users/jenkins/workspace/RELEASE__BuildPackages/build-EPpackage-macos/_CPack_Packages/Darwin/Bundle/arangodb3e-3.2.devel-1.x86_64/ArangoDB3e-CLI.app'; ${BUILDSCRIPT} "
    if (!Reliable) {
